...
void pushup(int x) {
    sum[x] = 0;
    for (int i = (x - 1) * siz + 1; i <= min(x * siz, n); i++) (sum[x] += a[i]) %= P;
}
void pushdown(int x) {
...
void updateadd(int l, int r, int k) {
    pushdown(bel[l]), pushdown(bel[r]);
    if (bel[l] == bel[r]) {
        for (int i = l; i <= r; i++) (a[i] += k) %= P;
        return pushup(bel[l]);
    }
    for (int i = l; bel[i] == bel[l]; i++) (a[i] += k) %= P;
    for (int i = r; bel[i] == bel[r]; i--) (a[i] += k) %= P;
    pushup(bel[l]), pushup(bel[r]);
    for (int i = bel[l] + 1; i <= bel[r] - 1; i++)
        (sum[i] += (i64)siz * k % P) %= P, (add[i] += k) %= P;
}
int query(int l, int r) {
    pushdown(bel[l]), pushdown(bel[r]);
    int ret = 0;
    if (bel[l] == bel[r]) {
        for (int i = l; i <= r; i++) (ret += a[i]) %= P;
        return ret;
    }
    for (int i = l; bel[i] == bel[l]; i++) (ret += a[i]) %= P;
    for (int i = r; bel[i] == bel[r]; i--) (ret += a[i]) %= P;
    pushup(bel[l]), pushup(bel[r]);
    for (int i = bel[l] + 1; i <= bel[r] - 1; i++) (ret += sum[i]) %= P;
    return ret;
}
int main() {
